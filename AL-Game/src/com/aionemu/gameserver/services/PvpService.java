...
public class PvpService {
        ...
        if (this.getKillsFor(winner.getObjectId(), victim.getObjectId()) < CustomConfig.MAX_DAILY_PVP_KILLS) {
            ...
            if (CustomConfig.ENABLE_TOLL_REWARD) {
                if (Rnd.get(0, 100) > CustomConfig.TOLL_CHANCE){
                    ...
                    PacketSendUtility.sendMessage(winner, "You've received " + CustomConfig.TOLL_QUANTITY + " tolls from PvP!");
                }
            }
        }
       ...
       // }
        ...
        if (CustomConfig.ENABLE_GP_LOSE){
            if (CustomConfig.ENABLE_GP_FIXED_LOSE){
                AbyssPointsService.addAGp(victim, 0, -CustomConfig.GP_LOSE);
            }else{
                AbyssPointsService.addAGp(victim, 0, -gpActuallyLost);
            }
        }
    }
    ...
    private boolean rewardPlayerGroup(Player victim, int totalDamage, AggroInfo aggro) {
        ...
        for (Player member : players) {
            ...
            if (CustomConfig.ENABLE_GP_REWARD){
                AbyssPointsService.addAGp(member, victim, 0, memberGpGain);
            }
    ...
    private boolean rewardPlayer(Player victim, int totalDamage, AggroInfo aggro) {
        ...
        if (CustomConfig.ENABLE_GP_REWARD) {
            AbyssPointsService.addAGp(winner, victim, 0, gpPlayerReward);
        }
