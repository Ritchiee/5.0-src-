...
public class SM_CHARACTER_LIST extends PlayerInfo {
     ...
     */
    public SM_CHARACTER_LIST(int resetState, int playOk2) {
    	...
    	if (resetState ==0){
	        ...
	        for (PlayerAccountData playerData : account.getSortedAccountsList()) {
	            ...
	            if (cbi != null && cbi.getEnd() > System.currentTimeMillis() / 1000) {
	                ...
	                writeB(new byte[204]); // unk 4.5.0.18
	            } else {
					...
					writeH(0); //unk
					writeD(DAOManager.getDAO(MailDAO.class).haveUnread(pcd.getPlayerObjId()) ? 1 : 0); //Mail Ammount
					writeD(DAOManager.getDAO(MailDAO.class).haveUnread(pcd.getPlayerObjId()) ? 1 : 0);; //Unread Mail Ammount
					writeD(0); // unk
					writeD(0); //unk
					writeD(0); //unk
					writeQ(BrokerService.getInstance().getCollectedMoney(pcd)); // collected money from broker
					writeB(new byte[126]); // unk 4.9
	            }
	        }
    	}
    }
