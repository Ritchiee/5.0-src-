...
public class GameServer {
    ...
    public static void main(String[] args) {
        ...
        Util.printSection("STIGMA SKILL TREE");
        ...
        if (CustomConfig.EFUNCTIONS_ENABLE) {
            FFAService.getInstance().Init();
            TVTService.getInstance().Init();
            ArenaService.getInstance().Init();
        }
        // Enable Recharger Portal
        if (CustomConfig.RECHARGER_PORTAL_ENABLE) {
            RechargerPortal.getInstance().Init();
        }
        // Enable Siege Portal
        if (SiegeConfig.SIEGEPORTAL_ENABLE) {
            SiegePortal.getInstance().Init();
            RechargerPortal.getInstance().Init();
        }
