...
public class EnchantItemAction extends AbstractItemAction {
    ...
    public void act(final Player player, final Item parentItem, final Item targetItem, final Item supplementItem, final int targetWeapon) {
    	ItemQuality targetQuality = targetItem.getItemTemplate().getItemQuality();
            ...
            case RARE:
            case LEGEND:
           	 if (targetItem.getEnchantLevel() >= 20 && parentItem.getItemTemplate().getCategory() == ItemCategory.ENCHANTMENT || targetItem.getEnchantLevel() >= 20 && parentItem.getItemTemplate().getCategory() == ItemCategory.AMPLIFICATION) {
                    PacketSendUtility.sendMessage(player, "You cannot enchant higher than 20");                 
                    return;
				}
           	 break;
            case UNIQUE:
            case EPIC:
            case MYTHIC:
             if (targetItem.getEnchantLevel() >= 25 && parentItem.getItemTemplate().getCategory() == ItemCategory.ENCHANTMENT || targetItem.getEnchantLevel() >= 25 && parentItem.getItemTemplate().getCategory() == ItemCategory.AMPLIFICATION) {
                    PacketSendUtility.sendMessage(player, "You cannot enchant higher than 25"); 
                    return;
				}
           	 break;
		default:
    ...
    private boolean isSuccess(final Player player, final Item parentItem, final Item targetItem, final Item supplementItem, final int targetWeapon) {
        if (CustomConfig.ENCHANT_ALWAYS_SUCCESS){
        	return true;
        }else{
