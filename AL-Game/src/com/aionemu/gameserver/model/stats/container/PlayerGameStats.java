...
public class PlayerGameStats extends CreatureGameStats<Player> {
    ...
    public Stat2 getMovementSpeed() {
        ...
        if (owner.isInPlayerMode(PlayerMode.RIDE)) {
        	...
        	Equipment equipment = owner.getEquipment();
            Item wingsEquip = equipment.getWingsEquiped();
            int wingSoaring = 0;
            if (wingsEquip != null && owner.isInPowerFlight()) {
                wingSoaring = wingsEquip.getItemTemplate().getWingStats().getSoaring();
            }
        	int powerflight = owner.isInPowerFlight() ? CustomConfig.POWERFLIGHT_FLYSPEED : 0;
            movementSpeed = getStat(StatEnum.FLY_SPEED, Math.round(pst.getFlySpeed() * 1000));
            movementSpeed.addToBonus( (powerflight * 1000) + wingSoaring);
        ...
        } else if (owner.isInPowerFlight()) {
        	movementSpeed = getStat(StatEnum.FLY_SPEED, CustomConfig.POWERFLIGHT_FLYSPEED);
        } else {
