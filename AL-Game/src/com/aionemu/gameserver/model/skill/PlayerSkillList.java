...
public final class PlayerSkillList implements SkillList<Player> {
    ...
    public boolean addStigmaSkill(Player player, int skillId, int enchantLevel) {
        return addSkill(player, skillId, enchantLevel, true, false, PersistentState.NEW);
    }
    ...
    private synchronized boolean addSkill(Player player, int skillId, int enchantLevel, boolean isStigma, boolean isLinked, PersistentState state) {
        PlayerSkillEntry existingSkill = isStigma ? stigmaSkills.get(skillId) : basicSkills.get(skillId);

        boolean isNew = false;
        if (existingSkill != null) {
            if (existingSkill.getSkillLevel() >= enchantLevel) {
                return false;
            }
            existingSkill.setSkillLvl(enchantLevel);
        } else {
            if (isStigma) {
                stigmaSkills.put(skillId, new PlayerSkillEntry(skillId, true,  false, enchantLevel, state));
            } else if (isLinked) {
                basicSkills.put(skillId, new PlayerSkillEntry(skillId, false, true, enchantLevel, state));
                linkedSkills.put(skillId, new PlayerSkillEntry(skillId, false, true, enchantLevel, state));
                isNew = true;

            } else {
                basicSkills.put(skillId, new PlayerSkillEntry(skillId, false, false, enchantLevel, state));
                isNew = true;
            }
        }
        if (player.isSpawned()) {
