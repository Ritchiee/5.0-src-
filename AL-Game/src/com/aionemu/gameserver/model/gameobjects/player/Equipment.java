...
public class Equipment {
    ...
    private void notifyItemEquipped(Item item) {
    	 if (item.getItemTemplate().getBonusSkill() !=null){
     		owner.getSkillList().addSkill(owner, item.getItemTemplate().getBonusSkill().getSkillId(), 1);
     	}else if (item.getItemSkinTemplate().getBonusSkill() !=null) {
     		owner.getSkillList().addSkill(owner, item.getItemSkinTemplate().getBonusSkill().getSkillId(), 1);
     	}
         

         checkEquipEnchantCount();
    ...
    private void notifyItemUnequip(Item item) {
    	if (item.getItemTemplate().getBonusSkill() !=null){
    		SkillLearnService.removeSkill(owner, item.getItemTemplate().getBonusSkill().getSkillId());
    	}else if (item.getItemSkinTemplate().getBonusSkill() !=null) {
    		SkillLearnService.removeSkill(owner, item.getItemSkinTemplate().getBonusSkill().getSkillId());
    	}
    	if (item.getBuffSkill() > 0){
    		SkillLearnService.removeSkill(owner, item.getBuffSkill());
    		owner.getEffectController().removeEffect(item.getBuffSkill());
    	}
    ...
    public Item unEquipItem(int itemUniqueId, long slot) {
        ...
        synchronized (equipment) {
            ...
            //log.info("Item UNique:" + itemUniqueId + " Item Slot: " +slot +" Return if Stigma: " + !StigmaService.notifyUnequipAction(owner, itemToUnequip) );
    ...
    public Item getWingsEquiped() {
        return equipment.get(ItemSlot.WINGS.getSlotIdMask());
    }
    ...
    private boolean verifyRankLimits(Item item) {
        ...
        if(CustomConfig.DISABLE_RANK_EQUIPMENT == true){
        	return true;
        }else{
    ...
    private void checkEquipEnchantCount(){
    	int armorEnchanted = 0;
		int weaponEnchanted = 0;
		int skillId = owner.getRace() == Race.ASMODIANS ? 4695 : 4694;
		...
		if (armorEnchanted >= 5 && weaponEnchanted >= 1) {
			if (owner.getSkillList().isSkillPresent(4694) || owner.getSkillList().isSkillPresent(4695)) {
				return;
			}			
			owner.getSkillList().addSkill(owner, skillId, 1);
			PacketSendUtility.sendPacket(owner, new SM_SKILL_LIST(owner, owner.getSkillList().getBasicSkills()));
		} else {
			if (owner.getSkillList().isSkillPresent(skillId)) {
				SkillLearnService.removeSkill(owner, skillId);
			}
			PacketSendUtility.sendPacket(owner, new SM_SKILL_LIST(owner, owner.getSkillList().getBasicSkills()));
		}
    }
