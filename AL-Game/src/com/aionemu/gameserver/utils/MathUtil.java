...
//side 0 for right and 1 for left side
    private static float getFrontDegree(float angle, int side)
    {
        if(side == 0)//direita
        {
            if(angle < 90)
...
//       float x1 = obj2.getX();
//       float y1 = obj2.getY();
//       float delta = tdx/length;
//       float angle;
//       if(directions == AreaDirections.FRONT )
//          angle = convertHeadingToDegree(obj1.getHeading());
//       else
//           angle = calculateAngleFrom(obj1,obj2);
//       m = (float)Math.tan(angle);
//       float y,x;
//       //if target too far away
////       if(getDistance(x0,y0,x1,y1)>length)
////            return false;
//       
//       //if target behind the cilinder
//       float angleCheck = calculateAngleFrom(obj1,obj2);
//       if(angle < 90 || angle > 270)
//       {
//           if(angleCheck < getFrontDegree(angle,0) && angleCheck > getFrontDegree(angle,1))
//               return false;
//               
//       }
//       else
...
//       
////       
////           if(!((getFrontDegree(angle,0) < angleCheck || 0 < angleCheck) && (getFrontDegree(angle,1) > angleCheck || )))
////               return false;
////       if(angle > 270)
////           if(!((getFrontDegree(angle,1) > angleCheck || 90 > angleCheck) && getFrontDegree(angle,0) < angleCheck))
////               return false;
////       if(!(getFrontDegree(angle,0) < angleCheck && getFrontDegree(angle,1) > angleCheck))
////           return false;
//////       
////       if(tdx==0)
////       {
...
////    if(tdx==0)
////    {
////            if(getDistance(tdy/m + x0, y0+tdy, x1, y1) <= radius && Math.abs(tdz) < radius)
////                return true;
////    }
////    else
////    {
////            if(getDistance(x0+tdx, m * (x0 +tdx  - x0) + y0, x1, y1) <= radius && Math.abs(tdz) < radius)
////                return true;
////    }
////    
////    return false;
...
//            delta = tdy/length;
//            if(tdy > 0){
//                 for(float i = 0; i <= tdy && res == false; i = i + delta)
//                      {
//                      
//                        x = i/m + x0 ;
//                        res = getDistance(x, y0+i, x1, y1) <= radius && Math.abs(tdz) < radius;
//                      }
//            }else
//                for(float i = 0; i >= tdy && res == false; i = i + delta)
//                 {
...
//       }else
//            if(tdx > 0){
//                for(float i = 0; i <= tdx && res == false; i = i + delta)
//                 {
//                     y = m * (x0 +i  - x0) + y0;
//                      res = getDistance(x0+i, y, x1, y1) <= radius && Math.abs(tdz) < radius;
//
//
//
//                 }
