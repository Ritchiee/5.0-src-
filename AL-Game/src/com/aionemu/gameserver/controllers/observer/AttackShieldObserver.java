...
public class AttackShieldObserver extends AttackCalcObserver {
   ...
   // private static final Logger log = LoggerFactory.getLogger(AttackShieldObserver.class);
    /**
     * @param percent
     * @param value
     * @param status
    ...
    public void checkShield(List<AttackResult> attackList, Effect attackerEffect, Creature attacker) {
    	/*log.info("Attacked SKill :" + effect.getSkillId() + " Skill Name :" + effect.getSkillName());
    	log.info("Attacker Skill : " + effect.getEffected());*/
        for (AttackResult attackResult : attackList) {
            ...
            if (shieldType == 2) {
				...
				absorbedDamage = (int) (damage * damageReduce/100f);

				MaxDamageAllowed -= absorbedDamage;

				if (absorbedDamage > 0) {
					attackResult.setShieldType(shieldType);
					attackResult.setShieldMp((int)(absorbedDamage *  (damageReduce/100f)));
	                this.effect.getEffected().getLifeStats().reduceMp((int)(absorbedDamage *  (damageReduce/100f)));
				}
